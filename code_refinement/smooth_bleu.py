...
normalize2 = [
    (r'([\{-\~\[-\` -\&\(-\+\:-\@\/])',
     r' \1 '),  # tokenize punctuation. apostrophe is missing
    (r'([^0-9])([\.,])',
     r'\1 \2 '),  # tokenize period and comma unless preceded by a digit
    (r'([\.,])([^0-9])',
     r' \1 \2'),  # tokenize period and comma unless followed by a digit
...
def score_cooked(allcomps, n=4, ground=0, smooth=1):
    ...
    for k in range(n):
        ...
        if smooth == 1 and k > 0:
            addsmooth = 1
        logbleu += math.log(correct + addsmooth +
                            sys.float_info.min) - math.log(guess + addsmooth +
                                                           sys.float_info.min)
        if guess == 0:
...
def bleu_fromstr(predictions, golds, rmstop=True):
    ...
    if rmstop:
        ...
        stopwords = open(os.path.join(pypath,
